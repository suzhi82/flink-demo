...
/**
 * Author: Cliff
 * Desc:  从MySQL 读取数据的工具类
 */
public class MySQLUtil {
    /**
     * mysql 查询方法，根据给定的class 类型返回对应类型的元素列表
     */
    public static <T> List<T> queryList(String sql, Class<T> clazz, Boolean underScoreToCamel) {
        ...
        try {
            ...
            conn = DriverManager.getConnection(
                    "jdbc:mysql://hadoop114:3306/gmall2021_realtime?characterEncoding=utf-8&useSSL=false",
                    "root",
                    "root");
            // 创建数据库操作对象
            ...
            while (rs.next()) {
                ...
                for (int i = 1; i <= md.getColumnCount(); i++) {
                    ...
                    if (underScoreToCamel) {
                        // 直接调用Google 的guava 的CaseFormat LOWER_UNDERSCORE
                        // 小写开头+下划线 -> LOWER_CAMEL 小写开头+驼峰
                        propertyName = CaseFormat.LOWER_UNDERSCORE.to(CaseFormat.LOWER_CAMEL, propertyName);
                    }
                    // 调用apache 的commons-bean 中的工具类，给Bean 的属性赋值
            ...
            e.printStackTrace();
            throw new RuntimeException("查询mysql 失败！");
        } finally {
            // 释放资源
            if (rs != null) {
                try {
